 ...
 * see coral-wandering-lantern.md. No real email infrastructure exists yet
 * (deferred — D5), so this just surfaces the affected bookings + their
 * token-secured /pay links for staff to send manually; the same call site
 * is where a real ESP integration would plug in later.
 */
...
export async function saveDeparture(
  ...
  if (departureId) {
    ...
    justBecameGuaranteed = existing.bookingAssurance !== "guaranteed" && data.bookingAssurance === "guaranteed";
