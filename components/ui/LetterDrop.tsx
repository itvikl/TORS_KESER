...
export default function LetterDrop({
  ...
  // Cumulative character count before each word, so the stagger delay
  // continues counting across word boundaries without mutating state
  // during render.
        ...
        <span key={wi}>
          <span className="inline-block overflow-hidden pb-[0.15em] align-bottom">
            {word.split("").map((ch, ci) => {
              const delay = startDelayMs + (wordStarts[wi] + ci) * staggerMs;
              return (
                <span
                  key={ci}
